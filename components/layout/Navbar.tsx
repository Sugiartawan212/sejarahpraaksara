...
export default function Navbar() {
  return (
    <div className="fixed top-6 left-1/2 -translate-x-1/2 w-[92%] max-w-5xl z-50">
      {/* 
        Rahasia efek kaca (Glassmorphism):
        1. bg-[#2A2A27]/70 -> Warna Graphite Earth dengan transparansi 70%
        2. backdrop-blur-md -> Memberikan efek buram/kaca
        3. border border-white/20 -> Garis putih transparan
      */}
      <nav className="bg-[#2A2A27]/70 backdrop-blur-md border border-white/20 rounded-full px-6 md:px-8 py-3 flex justify-between items-center shadow-2xl">
        ...
        {/* Menu Tengah (Desktop) */}
        {/* Mengarahkan Link ke ID section masing-masing di halaman */}
        <div className="hidden md:flex items-center gap-8 text-xs font-medium tracking-widest text-[#EEEBE4]/80 uppercase">
          <Link href="#tim" className="hover:text-white transition-colors">Tim Kami</Link>
          <Link href="#timeline" className="hover:text-white transition-colors">Materi</Link>
          <Link href="#galeri" className="hover:text-white transition-colors">Galeri</Link>
        </div>

        {/* Tombol CTA Kanan (Mengarahkan langsung ke Kuis) */}
        <Link
          href="#quiz"
